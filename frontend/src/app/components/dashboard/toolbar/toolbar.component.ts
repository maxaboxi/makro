...
export class ToolbarComponent implements OnInit, OnDestroy {
              ...
              (success) => {
                if (success) {
                  this.flashMessage.show(
                    this.translate.instant("FOOD_ADDED_SUCCESFULLY"),
                    {
  ...
  openSaveDayModal(content) {
    ...
    if (foodsAdded === 0) {
      this.flashMessage.show(
        this.translate.instant("NO_FOODS_IN_MEALS_ERROR"),
        {
          cssClass: "alert-danger",
          timeout: 2000,
        }
    ...
    this.subscriptions.add(
      this.auth.updateUserTargets(targets).subscribe(
        (res) => {
          if (res["success"]) {
            this.flashMessage.show(
              this.translate.instant("INFORMATION_SAVED"),
              {
                cssClass: "alert-success",
                timeout: 2000,
              }
  ...
  createPdf() {
    ...
    this.subscriptions.add(
      this.statisticsServcice
        .CreatePDF({ user: this.user.uuid, day: loaded.id })
        .subscribe(() => {
      ...
      this.dayService.getSavedDay(uuid).subscribe(
        (day) => {
          ...
          localStorage.setItem(
            "loadedDay",
            JSON.stringify({ id: uuid, name: day.name })
          );
        ...
        (error: Error) => {
          this.flashMessage.show(
            this.translate.instant("NETWORK_LOADING_ERROR"),
            {
              cssClass: "alert-danger",
              timeout: 2000,
            }
    ...
    this.modalService.open(content, { centered: true }).result.then(
      (result) => {
        if (result === "save") {
          ...
          if (
            this.amountToAddPortions &&
            this.selectedMeal.portions &&
            !this.amountToAddGrams
          ) {
  ...
  private calculateFoodValuesWithPortions() {
    if (
      this.amountToAddPortions > this.selectedMeal.portions ||
      this.amountToAddPortions < this.selectedMeal.portions
    ) {
      this.selectedMeal.foods.forEach((f) => {
        f.energy =
          (f.energy / this.selectedMeal.portions) * this.amountToAddPortions;
        f.protein =
          (f.protein / this.selectedMeal.portions) * this.amountToAddPortions;
        f.carbs =
          (f.carbs / this.selectedMeal.portions) * this.amountToAddPortions;
        f.fat = (f.fat / this.selectedMeal.portions) * this.amountToAddPortions;
        f.fiber =
          (f.fiber / this.selectedMeal.portions) * this.amountToAddPortions;
        f.sugar =
          (f.sugar / this.selectedMeal.portions) * this.amountToAddPortions;
        f.amount =
          (f.amount / this.selectedMeal.portions) * this.amountToAddPortions;
      });
    }
  }
  ...
  private calculateFoodValuesWithGrams() {
    ...
    const percentsOfTotal =
      (this.amountToAddGrams * 100) / this.amountTotal / 100;
    this.selectedMeal.foods.forEach((f) => {
